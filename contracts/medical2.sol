...
contract hospital2{
    ...
    function hospital1Patients(address hosp1_addr, address _Id) public view returns(bool _state,address _paId,string memory _paName,string[] memory _paRecords){
        hospital1 temp = hospital1(hosp1_addr);
       require(PatientInfo[msg.sender].state == true || patientToDoctor[_Id][msg.sender] == 1,"PATIENTS OR ACCESS_GRANTED_DOCTORS ONLY");
        ...
        _paName = temp.ret_name(_Id);
        _paRecords = temp.ret_records(_Id);
    }
    ...
    // Function to grant or revoke access to doctor ,so that the doctors with access can view the corresponding patients HealthRecords
...
//====================================================================
    // contract hospital1{
    //     function setDoctorDetails(bool _state,address _drId,string memory _name) public;
    //     function getDoctorDetails(address _Id) public  view returns(bool _state,address _drId,string memory _name) ;
    //     function setHealthRecordsDetails(string memory _paName, address _paId, string memory _prescription) public;
    //     function setPatientDetails(bool _state,address _paId,string memory _paName,string[] memory _paRecords) public;
    //     function getPatientDetails(address _Id) public view returns(bool _state,address _paId,string memory _paName,string[] memory _paRecords) ;
